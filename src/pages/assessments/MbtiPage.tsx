...
import { translateMBTIQuestions } from '@/lib/assessment-translations';
import { useAssessmentStore } from '@/store/assessment';
import { useLanguageStore } from '@/store/language';
import { AssessmentAnswer } from '@/types';

export default function MbtiPage() {  const navigate = useNavigate();
  ...
  // Get questions with translations based on current language
  const questions = translateMBTIQuestions(mbtiQuestions, currentLanguage);
  const currentQuestion = questions[currentQuestionIndex];
  const progress = ((currentQuestionIndex + 1) / questions.length) * 100;

  useEffect(() => {
    if (!currentSession || currentSession.type !== 'mbti') {
      startSession('mbti');
    }
  ...
  const handleNext = () => {
    ...
    if (currentQuestionIndex === questions.length - 1) {
      ...
      navigate('/assessments/mbti/results');
    } else {
  ...
  const handlePrevious = () => {
    if (currentQuestionIndex > 0) {
      setCurrentQuestionIndex(currentQuestionIndex - 1);
      const prevAnswer = answers.find(a => a.questionId === mbtiQuestions[currentQuestionIndex - 1].id);
  ...
  return (
    <div className="min-h-screen py-12">
      <div className="container max-w-4xl">
        ...
        >
          <div className="flex items-center justify-between mb-4">            <Button variant="ghost" asChild>
              <Link to="/assessments">
          ...
          <h1 className="text-3xl font-bold mb-4">{t('mbtiTitle')}</h1>
